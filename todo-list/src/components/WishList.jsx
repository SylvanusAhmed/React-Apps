...
const WishList = () => {
  ...
  const filteredTasks = searchResults.filter((searchResult) => searchResult.status === "wishlist");
    ...
    <main className=' overflow-y-auto  '>
      ...
      {filteredTasks.length > 0 ? (
        ...
        filteredTasks.map((searchResult) =>(
          ...
          <ul >
                <li key={searchResult.id} className='list-none shadow-md flex items-center justify-around gap-5 text-white mb-5 p-3 cursor-pointer hover:shadow-2xl'>
                <input 
                  type="checkbox"

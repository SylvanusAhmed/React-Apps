...
const Shopping = () => {
  ...
  const filteredTasks = searchResults.filter((searchResult) => searchResult.status === "shopping");
    ...
    <main className=' overflow-y-auto mt-[4rem]'>
      ...
      {filteredTasks.length > 0 ? (
        ...
        filteredTasks.map((searchResult) =>(
          ...
          <ul>
                <li key={searchResult.id} className='list-none  overflow-y-auto shadow-md flex items-center justify-around gap-5 text-white mb-5 p-3 cursor-pointer hover:shadow-2xl'>
                <input 
                  type="checkbox"
